...
def run_probe(config: ProbeConfig) -> ProbeArtifact:
    ...
    with start_wandb_run(
        ...
        coefficients_path = ensure_parent_dir(
            Path(config.output_path).with_suffix(".coefficients.npy")
        )
        ...
        tracker.log_path(config.output_path, artifact_type="probe-report")
        tracker.log_path(coefficients_path, artifact_type="probe-weights")
        return artifact
